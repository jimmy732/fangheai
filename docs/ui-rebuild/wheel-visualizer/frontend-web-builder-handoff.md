...
- Current backend mode: local 4174 adapter → BoxClaw Admin Gateway 8001; the current runtime reports mock/no image route, so the UI intentionally falls back to a labeled local layout preview.
...
4. BoxClaw server: accept the JSON data-URL bridge payload, apply the fixed prompt, use the sponsored policy, queue the async job and return output URLs behind `/api/v1/fbox/wheel-visualizer`.
...
| F-005 | Generating state snapshot | Generate shows pending animation | 4174 → 8001 adapter; mock returns 503 | Dialog remains closable | Orbit/progress rail | Mock route checked | passed locally |
| F-006 | Three result articles | Results show 3 angle cards and step 05 | Local fallback now labeled; live adapter ready | Result alt text/actions | Result grid | Live provider not enabled | passed locally |
