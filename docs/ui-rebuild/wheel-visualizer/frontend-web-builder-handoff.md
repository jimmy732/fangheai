...
- Primary journey: product detail → upload → frame → generate → review.
...
- Included components: entry card, isolated dialog, file staging, crop controls, generating state, three result cards and error recovery.
...
- Current backend mode: localhost layout mock; production adapter contract documented below.
...
| API boundary | Browser may call only `/api/wheel-visualizer/jobs` and its status route; provider calls stay in BoxClaw | `app.js:wheelVisualizerRemoteJob`, `feature-map.md` |
| Async states | upload, crop, generating, results, error; exactly three results required | `app.js:wheelVisualizerModal`, `feature-map.md` |
...
| Results | exactly three outputs, local-preview label, real BoxClaw label | Angle names and wheel reference alt text; retry/close actions |
...
3. `docs/ui-rebuild/wheel-visualizer/`: freeze the API, prompt, persistence and verification contracts before BoxClaw wiring.
4. BoxClaw server: implement the multipart upload, fixed prompt, sponsored policy, async job store and output URLs behind the documented routes.
...
| F-004 | Generating state snapshot | Generate shows pending animation | Local mock only; live adapter pending | Dialog remains closable | Orbit/progress rail | Not run | passed locally |
| F-005 | Three result articles | Results show 3 angle cards | Local mock only; live adapter pending | Result alt text/actions | Result grid | Not run | passed locally |
